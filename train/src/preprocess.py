...
def pad_corpus(input_sentences, labels_sentences):
	...
	for s in labels_sentences:
		...
		padded_label = [start_token] + padded_sentence + [stop_token] + [pad_token] * (max_length - len(padded_label) - 1)
...
def get_data(inputs_file, labels_file):
    # Read files
    inputs = tf.convert_to_tensor(read_data(inputs_file))
    labels = tf.convert_to_tensor(read_data(labels_file))
